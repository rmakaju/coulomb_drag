...
class DatFile:
    ...
    def has_columns(self, columns: List[str]) -> Optional[str]:
        """Return the first missing column name from the list, or None if all exist."""
    ...
    def get_data(self, x, y, z, x_order, y_order, varying_x=False, varying_y=False):
        ...
        #convert the column names to lowercase, becasuse x_order and y_order are case sensitive
        self.df.columns = [col.lower() for col in self.df.columns]
        self.sizes = {k.lower(): v for k, v in self.sizes.items()}
        x, y, z = x.lower(), y.lower(), z.lower()
        x_order, y_order = x_order.lower(), y_order.lower()
...
def create_kernel(x_dev: float, y_dev: float, cutoff: int, distr: str) -> np.ndarray:
    """Generate and return a normalized 2D kernel based on distribution type."""
    distributions = {
...
class Data2D:
    ...
    def __init__(self,
                 ...
                 y: np.ndarray,
                 z: np.ndarray,
                 equidistant: Tuple[bool, bool] = (False, False),
                 varying: Tuple[bool, bool] = (False, False)) -> None:
